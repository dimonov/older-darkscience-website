...
function do_smilies($text)
{
	...
	for ($i = 0; $i < $num_smilies; ++$i)
		$text = preg_replace("#(?<=.\W|\W.|^\W)".preg_quote($smiley_text[$i], '#')."(?=.\W|\W.|\W$)#m", '$1<img src="/template/images/smilies/'.$smiley_img[$i].'" width="15" height="15" alt="'.substr($smiley_img[$i], 0, strrpos($smiley_img[$i], '.')).'" />$2', $text);
...
function parse_signature($text)
{
	...
	$text = function_exists(pun_htmlspecialchars) ? pun_htmlspecialchars($text) : htmlentities($text, ENT_QUOTES);
